...
function getDefaultMenuData(): Menu[] {
    return [
      {
        id: 1,
        title: "Products",
...
const Header = ({ menuData: propMenuData }: HeaderProps = {}) => {
  const [menuData, setMenuData] = useState<Menu[]>(getDefaultMenuData());
  ...
  useEffect(() => {
    ...
    // Use provided menuData if available, otherwise fetch
    if (propMenuData && propMenuData.length > 0) {
      setMenuData(propMenuData);
    } else {
      fetchMenuData();
    }
  }, [propMenuData]);
